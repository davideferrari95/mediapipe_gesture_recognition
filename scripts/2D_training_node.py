...
############################################################
    ...
    def createTrainingDataframe(self):
        ...
        saved_positions = [f for f in listdir(f'{self.package_path}/database/Gestures/{self.gesture_file}/') 
                           if isfile(join(f'{self.package_path}/database/Gestures/{self.gesture_file}/', f))]
