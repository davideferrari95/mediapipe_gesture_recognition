...
class MediapipeStreaming(Node):
    ...
    def __init__(self):
        ...
        # ROS Initialization
        super().__init__('mediapipe_stream_node')
        ...
        self.declare_parameter('enable_objectron', False)
        self.declare_parameter('objectron_model', 'Shoe')
        self.declare_parameter('webcam', 0)
        self.declare_parameter('realsense', False)
        ...
        self.enable_face_detection = self.get_parameter('enable_face_detection').get_parameter_value().bool_value
        self.enable_objectron      = self.get_parameter('enable_objectron').get_parameter_value().bool_value
        self.objectron_model       = self.get_parameter('objectron_model').get_parameter_value().string_value

        # Read Webcam Parameter -> Webcam / RealSense Input
        self.webcam    = self.get_parameter('webcam').get_parameter_value().integer_value
        self.realsense = self.get_parameter('realsense').get_parameter_value().bool_value
        ...
        if not self.objectron_model in self.available_objectron_models:
            self.get_logger().error('ERROR: Objectron Model Not Available | Shutting Down...')
            rclpy.shutdown('ERROR: Objectron Model Not Available')

        # MediaPipe Publishers
        self.hand_right_pub = self.create_publisher(Hand, '/mediapipe_gesture_recognition/right_hand', 1)
        self.hand_left_pub  = self.create_publisher(Hand, '/mediapipe_gesture_recognition/left_hand', 1)
        self.pose_pub       = self.create_publisher(Pose, '/mediapipe_gesture_recognition/pose', 1)
        self.face_pub       = self.create_publisher(Face, '/mediapipe_gesture_recognition/face', 1)
    ...
    def initMediaPipeSolutions(self):
        ...
        # Declare ROS2 Parameters
        self.declare_parameter('/holistic/static_image_mode', False)
        self.declare_parameter('/holistic/model_complexity', 1)
        self.declare_parameter('/holistic/smooth_landmarks', True)
        self.declare_parameter('/holistic/enable_segmentation', False)
        self.declare_parameter('/holistic/smooth_segmentation', True)
        self.declare_parameter('/holistic/refine_face_landmarks', True)
        self.declare_parameter('/holistic/min_detection_confidence', 0.5)
        self.declare_parameter('/holistic/min_tracking_confidence', 0.5)
        self.declare_parameter('face_mesh_mode', 0)
        self.declare_parameter('/face_detection/min_detection_confidence', 0.5)
        self.declare_parameter('/face_detection/model_selection', 0)
        self.declare_parameter('/objectron/static_image_mode', False)
        self.declare_parameter('/objectron/max_num_objects', 5)
        self.declare_parameter('/objectron/min_detection_confidence', 0.5)
        self.declare_parameter('/objectron/min_tracking_confidence', 0.99)
        self.declare_parameter('/objectron/focal_length', [1,1])
        self.declare_parameter('/objectron/principal_point', [0,0])
        self.declare_parameter('/objectron/image_size', None)

        # Read Holistic Parameters
        static_image_mode        = self.get_parameter('/holistic/static_image_mode').get_parameter_value().bool_value
        model_complexity         = self.get_parameter('/holistic/model_complexity').get_parameter_value().integer_value
        smooth_landmarks         = self.get_parameter('/holistic/smooth_landmarks').get_parameter_value().bool_value
        enable_segmentation      = self.get_parameter('/holistic/enable_segmentation').get_parameter_value().bool_value
        smooth_segmentation      = self.get_parameter('/holistic/smooth_segmentation').get_parameter_value().bool_value
        refine_face_landmarks    = self.get_parameter('/holistic/refine_face_landmarks').get_parameter_value().bool_value
        min_detection_confidence = self.get_parameter('/holistic/min_detection_confidence').get_parameter_value().double_value
        min_tracking_confidence  = self.get_parameter('/holistic/min_tracking_confidence').get_parameter_value().double_value

        # Read FaceMesh Parameters (0: Contours, 1: Tesselation)
        face_mesh_mode = self.get_parameter('face_mesh_mode').get_parameter_value().integer_value
        ...
        min_detection_confidence = self.get_parameter('/face_detection/min_detection_confidence').get_parameter_value().double_value
        model_selection          = self.get_parameter('/face_detection/model_selection').get_parameter_value().integer_value
        ...
        static_image_mode        = self.get_parameter('/objectron/static_image_mode').get_parameter_value().bool_value
        max_num_objects          = self.get_parameter('/objectron/max_num_objects').get_parameter_value().integer_value
        min_detection_confidence = self.get_parameter('/objectron/min_detection_confidence').get_parameter_value().double_value
        min_tracking_confidence  = self.get_parameter('/objectron/min_tracking_confidence').get_parameter_value().double_value
        focal_length             = self.get_parameter('/objectron/focal_length').get_parameter_value().integer_array_value
        principal_point          = self.get_parameter('/objectron/principal_point').get_parameter_value().integer_array_value
        image_size               = self.get_parameter('/objectron/image_size').get_parameter_value().double_array_value
    ...
    def processHand(self, RightLeft:bool, handResults:HolisticResults, image:np.ndarray):
        ...
        or ((RightLeft == self.LEFT_HAND)  and (handResults.left_hand_landmarks))):
            ...
            for i in range(len(handResults.right_hand_landmarks.landmark if RightLeft else handResults.left_hand_landmarks.landmark)):
                ...
                # Append Keypoint
                hand_msg.keypoints.append(self.newKeypoint(handResults.right_hand_landmarks.landmark[i] if RightLeft else handResults.left_hand_landmarks.landmark[i], i+1, self.hand_landmarks_names[i]))
    ...
    def stream(self):
        ...
        while (self.realsense or self.cap.isOpened()) and rclpy.ok():
            ...
            # Compute FPS
            fps = int(1 / (self.get_clock().now() - start_time).nanoseconds * 1e9)
