...
class MediapipeStreaming:
    ...
    def __init__(self, webcam, enable_right_hand=False, enable_left_hand=False,
        ...
        self.hand_right_pub = rospy.Publisher(
            '/mediapipe_gesture_recognition/right_hand', Hand, queue_size=1)
        self.hand_left_pub = rospy.Publisher(
            '/mediapipe_gesture_recognition/left_hand', Hand, queue_size=1)
        self.pose_pub = rospy.Publisher(
            '/mediapipe_gesture_recognition/pose', Pose, queue_size=1)
        self.face_pub = rospy.Publisher(
            '/mediapipe_gesture_recognition/face', Face, queue_size=1)
        ...
        if not self.objectron_model in self.available_objectron_models:
            rospy.logerr(
                'ERROR: Objectron Model Not Available | Shutting Down...')
        ...
        if self.enable_right_hand:
            self.gesture_file += 'Right'
        if self.enable_left_hand:
            self.gesture_file += 'Left'
        if self.enable_pose:
            self.gesture_file += 'Pose'
        if self.enable_face:
            self.gesture_file += 'Face'
    ...
    def processPose(self, poseResults, image):
        ...
        if poseResults.pose_landmarks:
            ...
            for i in range(len(poseResults.pose_landmarks.landmark)):
                ...
                # Append Keypoint
                pose_msg.keypoints.append(self.newKeypoint(
                    poseResults.pose_landmarks.landmark[i], i, self.pose_landmarks_names[i]))
...
############################################################
    ...
    def processFaceDetection(self, faceDetectionResults, image):
        ...
        if faceDetectionResults.detections:
            ...
            # Draw Face Detection
            for detection in faceDetectionResults.detections:
...
############################################################
    ...
    def flattenKeypoints(self, pose_msg, left_msg, right_msg, face_msg):  # Mi servirà?
        ...
        pose = np.zeros(33 * 4) if pose_msg is None else np.array(
            [[res.x, res.y, res.z, res.v] for res in pose_msg.keypoints]).flatten()
        left_h = np.zeros(21 * 4) if left_msg is None else np.array(
            [[res.x, res.y, res.z, res.v] for res in left_msg.keypoints]).flatten()
        right_h = np.zeros(21 * 4) if right_msg is None else np.array(
            [[res.x, res.y, res.z, res.v] for res in right_msg.keypoints]).flatten()
        face = np.zeros(468 * 3) if face_msg is None else np.array(
            [[res.x, res.y, res.z, res.v] for res in face_msg.keypoints]).flatten()
    ...
    def getResults(self, image):
        ...
        if self.enable_face_detection:
            self.face_detection_results = self.face_detection.process(image)

        # Get Objectron Results from Mediapipe
        if self.enable_objectron:
            self.objectron_results = self.objectron.process(image)
    ...
    def processResults(self, image):
        ...
        if self.enable_left_hand:
            frame_msg.left_hand = self.processHand(
                self.LEFT_HAND,  self.holistic_results, image)

        # Process Right Hand Landmarks
        if self.enable_right_hand:
            frame_msg.right_hand = self.processHand(
                self.RIGHT_HAND, self.holistic_results, image)

        # Process Pose Landmarks
        if self.enable_pose:
            frame_msg.pose = self.processPose(self.holistic_results, image)

        # Process Face Landmarks
        if self.enable_face:
            frame_msg.face = self.processFace(self.holistic_results, image)

        # Flat all the keypoints
        sequence = self.flattenKeypoints(
            frame_msg.pose, frame_msg.left_hand, frame_msg.right_hand, frame_msg.face)

        # Process Face Detection
        if self.enable_face_detection:
            self.processFaceDetection(self.face_detection_results, image)

        # Process Objectron
        if self.enable_objectron:
            self.processObjectron(self.objectron_results, image)
    ...
    '''
        ...
        try:
            os.makedirs(os.path.join(
                f'{self.package_path}/database/3D_Gestures/{self.gesture_file}/', gesture))
        except:
            pass  # In teoria makedirs se la cartella esiste va già dritta

        # Create a number folder for each video of the current gesture
        try:
            os.makedirs(os.path.join(
                f'{self.package_path}/database/3D_Gestures/{self.gesture_file}/', gesture, str(video_number)))
        except:
            pass
        ...
        npy_path = os.path.join(f'{self.package_path}/database/3D_Gestures/{self.gesture_file}/',
                                gesture, str(video_number), str(self.framenumber))
    ...
    def npyfileFiller(self, gesture, video_number):
        ...
        if not video_number == '':
            ...
            if npyfilenumber < 40:
                ...
                data = np.load(os.path.join(f'{self.package_path}/database/3D_Gestures/{self.gesture_file}/',
                               gesture, str(video_number), str(npyfilenumber - 1)+".npy"))

                # Copy the last npy file to obtain 40 npy files to train the NN
                for i in range(40 - npyfilenumber):
                    np.save(os.path.join(f'{self.package_path}/database/3D_Gestures/{self.gesture_file}/',
                            gesture, str(video_number), str(npyfilenumber + i)), data)
...
############################################################
    ...
    def stream(self):
        ...
        current_gesture_number = 0        

        #Get the stat from the progress file
        try:
        ...
        for root, dirs, files in sorted(os.walk(self.path)):
            ...
            # I can't control the walk function root, it starts always from zero, so I can skip the first n subfolder
            #while 

            print("Current Gesture from walk:", root)

            print("Current Gesture Number:", current_gesture_number, "Current gesture progress number", current_gesture_progress_number)

            if current_gesture_number <= current_gesture_progress_number:
            ...
            for filename in sorted(files):
                ...
                print("current video from walk", filename)
...
def initROS(name, rate):
    ...
    global enable_face_detection_
    enable_face_detection_ = rospy.get_param('enable_face_detection', False)
    global enable_objectron_
    enable_objectron_ = rospy.get_param('enable_objectron', False)
    global objectron_model_
    objectron_model_ = rospy.get_param('objectron_model', 'Shoe')

    # Debug Print
    from termcolor import colored
    print(colored(f'\nFunctions Enabled:\n', 'yellow'))
    print(colored(f'  Right Hand: {enable_right_hand_}',
          'green' if enable_right_hand_ else 'red'))
    print(colored(f'  Left  Hand: {enable_left_hand_}\n',
          'green' if enable_left_hand_ else 'red'))
    print(colored(f'  Skeleton:   {enable_pose_}',
          'green' if enable_pose_ else 'red'))
    print(colored(f'  Face Mesh:  {enable_face_}\n',
          'green' if enable_face_ else 'red'))
    print(colored(f'  Objectron:       {enable_objectron_}',
          'green' if enable_objectron_ else 'red'))
...
if __name__ == '__main__':
    ...
    if enable_objectron_:
        MediapipeStream = MediapipeStreaming(
            webcam_, enable_objectron=True, objectron_model=objectron_model_)
    elif enable_face_detection_:
        MediapipeStream = MediapipeStreaming(
            webcam_, enable_face_detection=True)
    else:
        MediapipeStream = MediapipeStreaming(
            webcam_, enable_right_hand_, enable_left_hand_, enable_pose_, enable_face_)
