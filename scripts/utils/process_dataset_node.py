...
class MediapipeDatasetProcess:
  ...
  def __init__(self):
    ...
    self.gesture_path    = os.path.join(self.package_path, 'database/3D_Gestures', self.gesture_enabled_folder)
  ...
  def saveProcessedVideo(self, gesture, keypoints_sequence):
    ...
    # Gesture SaveFile
    gesture_savefile = os.path.join(f'{self.package_path}/database/3D_Gestures/{self.gesture_enabled_folder}/ToPad_file', f'{gesture}.pkl')
