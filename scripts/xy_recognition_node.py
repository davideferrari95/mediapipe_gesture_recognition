...
class GestureRecognition2D:
    ...
    def Recognition(self):
        ...
        print ("Pose riconosciute: ", pose_recognition_name)

        pose_recognition_prob = self.model.predict_proba(X)[0]

        print ("Con la probabilità: ", pose_recognition_prob)

        # Print the Recognized Gesture 
        Prob = max(pose_recognition_prob)
        if (Prob > self.recognition_precision_probability): print("Con probabilità soddisfacente il gesto è: ", pose_recognition_name)
